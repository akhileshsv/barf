...
type Bm struct{
	...
	Code, Endc, Dtyp                                 int
	Lx, Ly, Tx, Ty, Mx, My, Vx, Vy, Pu, Pfac         float64
	Lspan, Lbr, Tbr, Klx, Kly                        float64
	Lbl, Rbl                                         float64 //worthless
	Lcf, Tcf                                         float64 //unbraced length of comp. flange
	Llt                                              float64 //length between braces for lat-tor buckling
	Rez                                              []int
	Vals                                             [][]float64
	PSFs                                             []float64
	Maxs                                             []float64
	...
	DL, LL, WL                                       float64
	Theta                                            float64
	Nbm                                              float64 //no. of beams (use for floor qty counts)
	Ldcases                                          [][]float64
	Nspans                                           float64
	Lspans                                           []float64
	Start                                            []float64
	...
	Txtplots                                         []string
	Rezplots                                         string                                 
	Lsup, Rsup                                       string //left and right support indices
	Ljnt, Rjnt                                       string //left and right conn/joint indices
	Kostin                                           float64
	Mindx                                            int
	Ssec                                             kass.StlSec
	Ssecs                                            []kass.StlSec
...
func (b *Bm) Init()(err error){
	...
	if b.Code == 1{
		if len(b.PSFs) == 0{
			b.PSFs = []float64{1.5,1.5}
		}
	}
	if b.Tx == 0.0{b.Tx = 1.0}
	if b.Tcf == 0.0{b.Tcf = 1.0} 
	return
}

//Table generates an ascii table report for a Bm
func (b *Bm) Table(printz bool) (err error){
	...
	table.SetHeader([]string{"grade", "section name", "span(mm)", "dtyp", "lsb?", "code"})
	row := fmt.Sprintf("%v, %s, %.3f, %v, %v, %s", b.Grd, b.Sname, b.Lspan, dtmap[b.Dtyp], b.Lsb, codemap[b.Code])
	...
	if b.Prln{
		...
		row = fmt.Sprintf("%s, %.2f, %.2f, %.2f, %.2f",smin.Sstr,mwt,b.Kostin,b.Lspan,b.Kostin * mwt*b.Lspan/1e3/9.81)
		table.Append(strings.Split(row, ","))
		table.Render()
		...
		if len(b.Ssecs)>0{	
			...
			switch b.Code{
				case 2:
				table.SetHeader([]string{"no.","section", "wt(kg/m)","vd(kn)", "md(kn.m)", "fm(n/mm2)", "pm(n/mm2)",
					"fs(n/mm2)","ps(n/mm2)","def(mm)", "dperm(mm)","fwb(n/mm2)","fwc(n/mm2)","fwb(n/mm2)","pwb(n/mm2)"})	
				for i, s := range b.Ssecs{
					row = fmt.Sprintf("%v, %s, %.2f, %.2f,%.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f",i+1,s.Sstr,s.Wt/9.81,s.Vu/1e3,s.Mu/1e6,s.Fm,
						s.Pm,s.Fs,s.Ps,s.Dmax,s.Lspan/s.Defrat,s.Fwc,s.Pwc,s.Fwb,s.Pwb)	
					table.Append(strings.Split(row, ","))
					if mwt == 0.0 || mwt > s.Wt{
						mwt = s.Wt
						mdx = i
				...
				table.SetHeader([]string{"no.","section", "wt(kg/m)","vd(kn)", "vu(kn)", "mu(kn.m)", "mur(kn.m)", "def(mm)", "dperm(mm)","fwb(n/mm2)","fwc(n/mm2)"})	
				for i, s := range b.Ssecs{
					row = fmt.Sprintf("%v, %s, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f",i+1,s.Sstr,s.Wt/9.81,s.Vd/1e3,s.Vu/1e3,s.Mu/1e6,s.Mur/1e6,s.Dmax,s.Lspan/s.Defrat,s.Fwb,s.Fwc)	
					table.Append(strings.Split(row, ","))
					if mwt == 0.0 || mwt > s.Wt{
						mwt = s.Wt
						mdx = i
			...
			smin := b.Ssecs[mdx]
			//fmt.Println("mindx",b.Mindx,df.Elem(b.Rez[b.Mindx],1),b.Vals[b.Mindx][12])
			table.SetCaption(true, "quantity")
			table.SetHeader([]string{"section","wt(kg/m)","area","rxx","ixx","zxx","cost\n(rs)","total cost\n(rs)"})
			row = fmt.Sprintf("%s, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f",smin.Sstr,mwt/9.81,smin.Area,smin.Rxx,smin.Ixx,smin.Zxx,b.Kostin,b.Kostin * mwt*b.Lspan/1e3/9.81)
			table.Append(strings.Split(row, ","))
			table.Render()
			rezstr.WriteString(ColorReset)
		} else {
			err = fmt.Errorf("empty section slice %v",len(b.Ssecs))
			return
		}
...
func (b *Bm) GenMod(ss kass.StlSec)(mod kass.Model, err error){
	...
	mod.Dims = b.Dims
	mod.Sts = append(mod.Sts, b.Bstyp)
	mod.Term = b.Term
	mod.Web = b.Web
	mod.Units = "nmm"
	mod.Drawsec = true
...
//ChkSec checks a beam section for uvals
func (b *Bm) ChkSec()(err error){
	...
	if b.Dtyp != 0{
		mod, frmrez, bmrez, err = b.Calc1d(ss)
		if err != nil{
			return
		}
		err = b.SortRez(bmrez)
	...
	ss.Vu = math.Abs(b.Vu); ss.Mu = math.Abs(b.Mu); ss.Dmax = math.Abs(b.Dmax)
	ss.Rl = math.Abs(b.Rl); ss.Rr = math.Abs(b.Rr)
	...
	if err == nil{
		if b.Drawrez && b.Dtyp != 0 && len(b.Ssecs) == 0{
			//draw first bm sf rez
			switch b.Dtyp{
				case -1, 1:
				pltchn := make(chan string,1)
				go kass.PlotBm1d(&mod, b.Term, pltchn)
				pltstr := <-pltchn
				b.Txtplots = append(b.Txtplots, pltstr)

				pchn := make(chan string,1)
				go kass.PlotSfBmDx(-1, b.Lspan, bmrez[1],b.Title,b.Term,b.Web,pchn)
			 	pstr := <- pchn
				b.Txtplots = append(b.Txtplots, pstr)
				
				case 2:
				//cs beam
				mod.CalcRez("1db", frmrez)
				b.Txtplots = append(b.Txtplots,kass.PlotRez2d(&mod, b.Term))
		...
		b.Ssecs = append(b.Ssecs, ss)
	}
	return
}

//SortRez sorts a beamrez for uvals
...
func (b *Bm) Calc1d(ss kass.StlSec)(mod kass.Model, frmrez []interface{},bmrez map[int]kass.BeamRez, err error){
	mod, err = b.GenMod(ss)
	if err != nil{
		return
	}
...
//BmDz 'Dz a Bm' new and improved ss. StlSec way
...
func StlBmDBs(lspan, ly, ty, lbr, tbr float64, ldcases [][]float64, sname string, grd, nsecs int, brchck, yeolde bool) []int {
	...
	for i := df.Nrow() - 1; i > 0; i-- {
		...
		log.Println(ColorYellow, "dtrat",dtrat,"sdrat",sdrat,"ryy",df.Elem(i, 14).Float(),"cm3","px",px,ColorReset)
		...
		log.Println(ColorGreen,"shear-",maxs[0],ColorReset)
...
func (b *Bm) TableOlde(printz bool) {
	...
	if printz{
		fmt.Println(b.Report)
			...
			// switch b.Dtyp{
			// 	case -1, 1:
			// 	go kass.PlotSfBmDx(1, b.Lspan, bmrez[1],b.Title,b.Term,b.Web,pltchn)
			// 	pltstr := <- pltchn
			// 	ss.Rezplot = pltstr
			// 	//if b.Term == "dumb"{fmt.Println(pltstr)}
			// 	case 2:
			// 	if len(bmrez) == len(b.Lspans){
			// 		for i:= 1; i <= len(bmrez); i++{
			// 			go kass.PlotSfBmDx(i, b.Lspan, bmrez[i],b.Title,b.Term,b.Web,pltchn)
			// 			pltstr := <- pltchn
			// 			ss.Rezplots = append(ss.Rezplots, pltstr)
			// 		}
			// 	}
			// }
*/
