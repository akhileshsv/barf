...
func (b *Bm) Table(printz bool) {
	...
	if b.Dz{
		...
		for i, idx := range b.Rez{
			...
			row = fmt.Sprintf("%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", sname,vu, mu, dmax, dperm, px, fx, ps, fs, pc, fc, pb, fb)
			table.Append(strings.Split(row, ","))
		}
		table.Render()
		rezstr.WriteString(ColorCyan)
...
//BmDesign designs a steel beam (calls BmDz)
func BmDesign(b *Bm) (err error) {
	//return
	log.Println(ColorYellow, "***insert stl beam design here***", ColorReset)
	err = BmDz(b)
	return
}

//BmDz is the entry func for steel beam design
//TODO - write continuous beam calcs
func BmDz(b *Bm) (err error) {
	b.Mindx = -1
	df, err := kass.GetStlDf(b.Styp)
	if err != nil {
		return
	}
	if b.Grd == 0 {
...
func (b *Bm) Chk449(df dataframe.DataFrame, i int) (vals []float64, chk bool) {
	...
	if b.Yeolde{
		...
		px = PbcLerp(b.Styp, b.Grd, sdrat, dtrat)
	}
...
func SimpBmFrc(member int, ldcases [][]float64, lspan, e, area, ix float64) []float64 {
	...
	rez := kass.Bmsfcalc(1, ldcases, lspan, e, area, ix, rl, 0, rr, 0, false)
...
//old func, ignore
func StlBmDBs(lspan, ly, ty, lbr, tbr float64, ldcases [][]float64, sectyp, grd, nsecs int, brchck, yeolde bool) []int {
	memid := ldcases[0][0]
	mem := int(memid)
	...
	for i := df.Nrow() - 1; i > 0; i-- {
		...
		if yeolde {
			...
			px = PbcLerp(sectyp, grd, sdrat, dtrat)
		}
