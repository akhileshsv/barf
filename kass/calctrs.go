...
func CalcTrs(mod *Model, ncjt int) ([]interface{},error) {
	...
	//log.Println(ColorCyan,"GLOBAL NODAL DISPLACEMENT VECTOR\n",dglb,ColorReset)
	...
	mod.Frmtyp = 4
	if ncjt == 2 {
		tstr := Trs2dTable(js,ms,dglb,rnode,nsc,ndof,ncjt)
		tstr += FrcTable(mod)
		frmrez[6] = tstr
		mod.Frmtyp = 1
	}
	if ncjt == 3 {frmrez[6] = Trs3dTable(js,ms,dglb,rnode,nsc,ndof,ncjt)}
	frmrez[7] = p
	frmrez[8] = d0
	frmrez[9] = schmat
