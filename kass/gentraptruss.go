			...
			if nrs %2 == 0 && nrs > 2 {
				...
				for i, node := range bcns{
					...
					case node < nrs/2:
						for je := i + node + nrs -1; je < i +node + nrs + 2; je++ {
							if _, ok := tcnodes[je]; ok{ ms = append(ms, []int{node, je})}
						}
					case node == nrs/2:
						for je := i + node + nrs -1; je < i +node + nrs + 1; je++ {
							if _, ok := tcnodes[je]; ok{ ms = append(ms, []int{node, je})}
						}
						ms = append(ms, []int{node, i+node+nrs+2})
						midx := (coords[i+node+nrs+2-1][0] + coords[node-1][0])/2.0
						midy := (coords[i+node+nrs+2-1][1] + coords[node-1][1])/2.0
						coords = append(coords, []float64{midx,midy})
						ms = append(ms, []int{i+node+nrs,len(coords)})
						ms = append(ms, []int{i+node+nrs+1,len(coords)})
					case node == (nrs/2) + 1:
						for _, je := range []int{i + node + nrs, i + node + nrs + 2, i + node + nrs + 3} {
							if _, ok := tcnodes[je]; ok{ ms = append(ms, []int{node, je})}
						}
						ms = append(ms, []int{node, i+node+nrs+2})
						...
						coords = append(coords, []float64{midx,midy})
						ms = append(ms, []int{i+node+nrs+2,len(coords)})
						ms = append(ms, []int{i+node+nrs+1,len(coords)})				
					default:
				...
				for i, node := range bcns {
					...
					case node == (nrs/2) + 1:
						...
						ms = append(ms, []int{i+node+nrs+1,len(coords)})
					case node == (nrs/2) + 2:
						for _, je := range []int{i + node + nrs, i + node + nrs + 2} {
							if _, ok := tcnodes[je]; ok{ ms = append(ms, []int{node, je})}
						}
						midx := (coords[i+node+nrs-1][0] + coords[node-1][0])/2.0
						midy := (coords[i+node+nrs-1][1] + coords[node-1][1])/2.0
