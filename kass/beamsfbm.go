...
func BeamFrc(ncjt, member int, mem *Mem, ldcases [][]float64, spanchn chan BeamRez, plotbm bool) {
	...
	case ncjt == 2://beam
		...
		iz = mem.Geoms[2]
		ar = mem.Geoms[3]
		r = Bmsfcalc(member, ldcases, l, e, ar, iz, rl, ml, re, me, plotbm)
	case ncjt == 3://frame
		rl = mem.Qf[1]
		ml = mem.Qf[2]
		re = mem.Qf[4]
...
func Bmsfcalc(member int, ldcases [][]float64, l, e, a, iz, rl, ml, re, me float64, plotbm bool) (BeamRez){	
	//fmt.Println("mem->",member,"rl,ml,re,me->",rl,ml,re,me)
	//mfrtyp := 0
	ndiv := 21
	nd := float64(ndiv) - 1.0
	div := l/(nd)
	...
	for i, x := range xs {
		...
		if math.Abs(dmax) < math.Abs(dxs[i]*1000) {
			dmax = 1000 * dxs[i]
			dmaxx = x
			dxs[i] = mdxs[i] + vdxs[i]
		}
