...
type Model struct {
	...
	Clvrs    [][]float64 //cantilevers - is this needed?
	...
	Mtyp     []int //1 - rcc slab ?? not used
	Mtprp    [][]float64
	Lsx      []float64
	Nlp      int //same as below lmao
	Nlc      int //no. of load combos
	Nlt      int //no. of load types
	...
	Reports  []string
	Txtplots []string
	Mldsrv   map[float64][][]float64
	Jldsrv   map[float64][][]float64
	Mldcs    map[float64][][]float64
	Jldcs    map[float64][][]float64 
	Bmenv    map[int]*BmEnv
	Ms       map[int]*Mem
	Js       map[int]*Node
