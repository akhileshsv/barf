...
func CalcSecProp(styp int, dims []float64) (bar Secprop) {
	...
	var area, perimeter, ixx, iyy, xc, yc, sxx, syy, zxx, zyy, j, rxx, ryy float64
	...
	if rxx == 0.0{rxx = ixx/area}
	if ryy == 0.0{ryy = iyy/area}
	if j == 0.0{j = ixx + iyy}
	bar = Secprop{
		Area:      area,
		Perimeter: perimeter,
...
/*
func Between(ptx, pta, ptb pt) bool {
	//collinear already, right?
	
	//if ab non vertical, check on x, else check on y
	if pta.x != ptb.x {
		return ((pta.x <= ptx.x) && (ptx.x <= ptb.x)) || ((pta.x >= ptx.x) && (ptx.x >= ptb.x))
	} else {
		return ((pta.y <= ptx.y) && (ptx.y <= ptb.y)) || ((pta.y >= ptx.y) && (ptx.y >= ptb.y))
	}
}

func lieson(ptx, pta, ptb pt) bool {
	epsilon := 0.01
	crsprd := (ptx.y - pta.y) * (ptb.x - pta.x) - (ptx.x - pta.x) * (ptb.y - pta.y)
	dotprd := (ptx.x - pta.x) * (ptb.x - pta.x) + (ptx.y - pta.y) * (ptb.y - pta.y)
	if math.Abs(crsprd) > epsilon{
		return false
	}
	if dotprd < 0.0 {
		return false
	}
	sqrlen := (ptb.x - pta.x) * (ptb.x - pta.x) + (ptb.y - pta.y) * (ptb.y - pta.y)
	return dotprd - sqrlen < 0.0
}
   rect section
		   ???
		if b <= d {
			...
			j = math.Pow(d, 3) * b * ((1.0 / 3.0) - 0.21*(d/b)*(1.0-(1.0/12.0)*math.Pow((d/b), 4)))
		}
