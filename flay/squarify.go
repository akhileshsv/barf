...
func (f *Flr) FlrGen()(err error){
	...
	switch f.Togrid{
		...
		f.GenWalls(rmap, nmap, ptmap, wmap, pts)
		f.GenPlan(rmap, nmap, ptmap, wmap, pts)
		case true:
		grid := f.PolyGrid()
		f.CorGrid(grid)
	}
...
//GenWalls generates walls/door openings
func (f *Flr) GenWalls(rmap map[int]*Rm, nmap map[Pt][]*Wall, ptmap map[Pt][]int, wmap map[Tupil][]int, pts []Pt)(err error){
	...
	//mark main door
	for dir := range rmap[ldx].Walls{
		if dir < 0{
			dwall := rmap[ldx].Walls[dir][0]
			dwall.Typ = 2
			jb := ptmap[*dwall.Pb][0]; je := ptmap[*dwall.Pe][0]
			edx := EdgeDx(jb, je)
			wmap[edx][0] = 2
			break
		}
