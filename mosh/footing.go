...
func (f *RccFtng) Printz() (rez string){
	...
	if f.Dz{
		...
		rez += fmt.Sprintf("mux, vux, vp, rbr x- %.2f, %.2f, %.2f, %.0f, %.0f, %.0f, %.0f x\n",f.Mux,f.Vux,f.Vp,f.Rbr[0],f.Rbr[2],f.Rbr[3],f.Rbr[4])
		rez += fmt.Sprintf("muy, vuy, vp, rbr y- %.2f, %.2f, %.2f, %.0f, %.0f, %.0f, %.0f y\n",f.Muy,f.Vuy,f.Vp,f.Rbr[0],f.Rbr[6],f.Rbr[7],f.Rbr[8])
	}
	return
}
