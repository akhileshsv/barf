...
var (
	...
	CostStl float64 = 75.0 * 1.38//0.01515
...
func Quant(barr[]*RccBm)(err error){
	...
	switch barr[1].Styp{
		case 1:
		barr[1].Vrcc = lspan * (barr[1].Bw * barr[1].Dused) * 1e-6 - lspan * (barr[1].Bw * barr[1].Dslb) * 1e-6
		barr[1].Afw = lspan * (barr[1].Sec.Prop.Perimeter) * 1e-3 - barr[1].Bw * lspan * 1e-3 - barr[1].Dslb * lspan * 1e-3 * 2.0
		case 6,7,8,9,10:
		barr[1].Vrcc = lspan * (barr[1].Sec.Prop.Area) * 1e-6 - lspan * (barr[1].Bf * barr[1].Dslb) * 1e-6 
		//barr[1].Afw = lspan * (barr[1].Sec.Prop.Perimeter - 2.0 * barr[1].Df - barr[1].Bf) * 1e-3
		barr[1].Afw = lspan * (barr[1].Bf + 2.0 * (barr[1].Dused - barr[1].Df)) * 1e-3
		lxp = 2.0 * ((barr[1].Bw - 60.0)+(barr[1].Dused-60.0))	
		default:
		barr[1].Vrcc = lspan * (barr[1].Sec.Prop.Area) * 1e-6
		barr[1].Afw = lspan * (barr[1].Sec.Prop.Perimeter) * 1e-3
		//just use square stirrups? get bw sec at c.Cvrt, c.Cvrc
		//(or) get linksec and get perimeter is the way
		//lxp = 2.0 * ((barr[1].Bw - 60.0)+(barr[1].Dused-60.0))
	}
	//add dev len to l1/l3 for ascz
	//curtail 50% of midspan (at WHAAT)
	var ldreq float64
	switch barr[1].Endc{
		case 0:
		dia := barr[1].Dia3
...
func (b *RccBm) Table(printz bool){
	...
	*/
	...
	row = fmt.Sprintf("M%.f,Fe%.f,Fe%.f,%.2f,%.2f",b.Fck,b.Fy,b.Fyv,b.Cvrc,b.Cvrt)
	...
	if b.Shrdz{
		...
		for i, typ := range []string{"main","min","nominal"}{
			...
			l1 := 0.0; l2 := b.L1; l3 := b.L4; l4 := b.Xs[20]
			switch i{
				case 0:
				case 1:
				ltot = minlen
...
func (b *RccBm) Draw() (data string, err error){
	...
	switch b.Styp{
		...
		links = [][]float64{
			{b.Bf/2.0 - b.Bw/2.0 + 30.0,30.0},
			{b.Bf/2.0 + b.Bw/2.0 - 30.0,30.0},
			{b.Bf/2.0 + b.Bw/2.0 - 30.0, b.Dused - 30.0},
			{b.Bf/2.0 - b.Bw/2.0 + 30.0, b.Dused - 30.0},
			{b.Bf/2.0 - b.Bw/2.0 + 30.0,30.0},
		}
		ldata += fmt.Sprintf("%f %f %.f\n",0.0, b.Dused/2.0, b.Dused)
		...
		links = [][]float64{
			{b.Bw/2.0 - b.Bf/2.0+30.0,30.0},
			{b.Bw/2.0 + b.Bf/2.0-30.0,30.0},
			{b.Bw/2.0 + b.Bf/2.0-30.0,b.Dused-30.0},
			{b.Bw/2.0 - b.Bf/2.0+30.00,b.Dused-30.0},
			{b.Bw/2.0 - b.Bf/2.0+30.0,30.0},
		}
		ldata += fmt.Sprintf("%f %f %.f\n",0.0, b.Dused/2.0, b.Dused)
		ldata += fmt.Sprintf("%f %f %.f\n",b.Bw/2.0, 0.0, b.Bw)
...
func (b *RccBm) BarLay() (err error){
	...
	switch b.Styp{
		case 1:
		xt = 30.0; yt = b.Cvrt; xc = 30.0; yc = b.Dused - b.Cvrc
		case 7:
		xt = 30.0; yt = b.Cvrt; xc = 30.0; yc = b.Dused - b.Cvrc
		case 6:
		xt = 30.0 + b.Bf/2.0 - b.Bw/2.0; yt = b.Cvrt; xc = xt; yc = b.Dused - b.Cvrc
		case 14:
		xt = 30.0 + b.Bw/2.0 - b.Bf/2.0; yt = b.Cvrt; xc = xt; yc = b.Dused - b.Cvrc
		default:
		//HUH? HUH?
	}
	if b.Ast > 0.0{
		...
		switch n1 + n2{
			...
			switch b.Styp{
				case 14:
				xstep = (b.Bf - 2.0 * 30.0)/2.0
				default:
				xstep = (b.Bw - 2.0 * 30.0)/2.0
			}
			default:
			switch b.Styp{
				case 14:
				xstep = (b.Bf - 2.0 * 30.0)/float64(n1+n2-1)
				default:
				xstep = (b.Bw - 2.0 * 30.0)/float64(n1+n2-1)
			}
		}
