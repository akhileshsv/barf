...
//why define a separate CBm struct and functions this is this worst form of laziness
...
type CBm struct {
	...
	Base         *kass.Model `json:",omitempty"` 
...
func CBeamEnvRcc(cb *CBm, termstr string, report bool)(bmenv map[int]*kass.BmEnv, err error){
	...
	*/
	...
	copy(cb.Bmvec, bmvec)
	cb.Txtplots = make([]string, cb.Nspans+4)
	if termstr != "" {
		//PLOT ALL LOAD PATTERNS - add new bool for this
		if termstr == "dumb"{
