...
//GenCoords generates SubFrm coords based on sf.Lspans and sf.H 
...
func (sf *SubFrm) GenLoads() (err error){
	...
	if len(sf.Bloads) != 0 {
		for _, ldcase := range sf.Bloads{
			mem := int(ldcase[0]) + len(sf.H)*colstep
	...
	if sf.Lclvr{
		...
		for mem := range sf.Benloads {
			switch mem{
				case sf.Lcmem, 1+len(sf.H)*colstep:
	...
	if sf.Rclvr{
		...
		for mem := range sf.Benloads {
			rmem := len(sf.H)*colstep + sf.Nspans
			switch mem{
				case sf.Rcmem, rmem:
...
func (sf *SubFrm) InitMemRez() (err error){
	...
	for _, i := range sf.Beams{
		...
		switch i{
			case sf.Lcmem:
			rsx = sf.Secmap[1][1]/2.0
			bmendc = 0
			case sf.Rcmem:
			lsx = sf.Secmap[colstep][1]/2.0
			bmendc = 0
			default:
			lsx = sf.Secmap[cl][1]/2.0; rsx = sf.Secmap[cr][1]/2.0
		}
...
func (sf *SubFrm) DrawMod(){
	...
	sf.Txtplots = append(sf.Txtplots, PlotBmEnv(sf.Bmenv, sf.Beams, sf.Term, sf.Title, sf.Foldr))
	if sf.DM > 0.0{
	...
	for i := 0; i <= sf.Nlp; i++{
		go PlotLp(i, sf.Bmenv, sf.Ms, sf.Mslmap, sf.Beams, sf.Term, sf.Title, sf.Foldr,plotchn)
	}
	for i := 0; i<=sf.Nlp; i++{
		rez := <- plotchn
		//lp, _ := rez[0].(int)
...
//it does nothing, just tries very hard and fails
