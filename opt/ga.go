...
func selecprop(pop []Bat, mx int) (pool []Bat, gb, gw, avg float64, gpos []int){
	...
	//fmt.Println("prop",len(pool),len(pop))
	return
}

func Gabloop(par, ndchk bool, np, ng, mt, ct, cn, st, mx int, pmut, pcrs float64, nd []int, inp []interface{},f Bobj, drw, trm, title string) ([]int, float64, float64, error){
	//basic ga loop
	rand.Seed(time.Now().UnixNano())	
	var pool []Bat
	var gb, gw, gavg float64
	var gpos []int
	var fdat, wdat string
	var gavs []float64
	pop, err := Bpopgen(par, np, nd, inp, f)
	if err != nil{
		return gpos, gw, gb, err
	}
	for gen := 0; gen < ng; gen++{
		...
		switch drw{
			...
			default:
			if gen % 10 == 0{
				fmt.Println(ColorBlue, "gen->",gen,ColorWhite, ColorGreen,"\tmax fitness",gb,"\tweight->", gw, "\tavg",gavg,ColorReset)
			}
		}
		pop, err = Brepop(par, ndchk, np, mt, ct, cn, mx, pmut, pcrs, nd, inp, pool, f)
		if err != nil{return gpos, gw, gb, err}
	}
	skript := "d2.gp"
	var dstr string
	switch drw{
		case "gen":
		dstr, _ = draw.Dumb(fdat, skript, trm, title, "gen", "fit", "")
		if trm != "qt" {fmt.Println(dstr)}
		if mx == -1{
			dstr, _ = draw.Dumb(wdat, skript, trm, title, "gen", "wt", "")
			if trm != "qt" {fmt.Println(dstr)}
		}
	}
	fmt.Println(ColorCyan, "best pos->\n", gpos, ColorRed, "\nmax fitness->", gb, ColorReset)
	if mx == -1{fmt.Println(ColorCyan, "min weight->", gw, ColorReset)}
	return gpos, gw, gb, nil
}
...
func Gabaloop(par, ndchk bool, np, npmn, npmx, ngmn, ngmx, mt, ct, cn, st, mx, step int, pmut, pcrs, gap float64, nd []int, inp []interface{},f Bobj, drw, trm, title string) ([]int, float64, error){
	rand.Seed(time.Now().UnixNano())	
	var pool []Bat
	...
	for gen := 0; gen < ngmx; gen++{
		if gen > ngmn && !isimp(gavs, 50, gap){
			fmt.Println("no improvement for 50, stopping")
			break
		}
		if gen > step{
		...
		if np < npmn {np = npmn}
		if np > npmx {np = npmx}
		if pmut < 0.01{pmut = 0.01}
		//if pmut > 1.0{pmut = 1.0}
		if pcrs < 0.1{pcrs = 0.1}
		if pcrs > 1.0{pcrs = 0.99; pmut = 0.01}
		if pmut > 1.0{pmut = 0.9; pcrs = 0.1}
