...
func TestPsoBasic(t *testing.T){
	...
	Psoloop(par, w, c1, c2, np, ng, nd, mx,mn, rastapso, inp, drw, trm, title)
	...
	Psoloop(par, w, c1, c2, np, ng, nd, mx,mn, spherepso, inp, drw, trm, title)
...
func TestTrsPso(t *testing.T) {
	...
	for idx, ex := range examples{
		...
		gpos, gb := Psoloop(par, w, c1, c2, np, ng, nd, mx,mn,trsrakapso, inp, drw, trm, title)
		fmt.Println(gpos, gb)
 		//ipos := make([]int, nd)
