...
func TestGaUnemax(t *testing.T){
	...
	_, _, _, err := Gabloop(par, ndchk, np, ng, mt, ct, cn, st, mx, pmut, pcrs, nd, inp, f, drw, trm, title)
...
func TestGaKnpsck(t *testing.T){
	...
	gpos, _, _, _ := Gabloop(par, ndchk, np, ng, mt, ct, cn, st, mx, pmut, pcrs, nd, inp, f, drw, trm, title)
	wt := 0. 
	for i := range items{
		if gpos[i] == 1{
...
func TestARaka(t *testing.T) {
	...
	for idx, ex := range examples{
		...
		_, _, err := Gabaloop(par, ndchk, np, npmn, npmx, ngmn, ngmx, mt, ct, cn, st, mx, step, pmut, pcrs, gap, nd, inp, f, drw, trm, title)
		if err != nil{fmt.Println(err)}
	}
}
...
func TestRakaTruss(t *testing.T) {
	...
	for idx, ex := range examples{
		...
		bb := -100.0
		for i := 0; i < 5; i++{
			gpos, gw, gb, err = Gabloop(par,ndchk, np, ng, mt, ct, cn, st, mx, pmut, pcrs, nd, inp, f, drw, trm, title)
	...
	for mt := range mtz{
		for ct := range ctz{
			for st := range stz{
				...
				fmt.Println("params->",ColorCyan,"mut-",mtz[mt],"\tcrs-",ctz[ct],"\tsel-",stz[st])
				fmt.Println(ColorReset)
				gpos, _, gb, err = Gabloop(par, ndchk, np, ng, mt, ct, cn, st, mx, pmut, pcrs, nd, inp, f, drw, trm, title)
				if err != nil{
					fmt.Println(err)
				} else {
					fmt.Println(gpos)
