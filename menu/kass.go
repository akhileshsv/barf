	...
	for running{
		...
		switch choice{
			...
			switch choice{
				case 0,1,2:
				//calcbm1d
				err = kass.CalcMod(mod, "1db", term)
				case 3:
				//non uniform beam
				err = kass.CalcModNp(mod, "1db", term)
				case 4,5:
				//beam sf/bm
	...
	for running{
		...
		switch choice{
			...
			switch choice{
				...
				switch choice{
					case 0,1,2,3:
					err = kass.CalcMod(&mod,"2df",term)
					case 4:
					err = kass.CalcModNp(&mod, "2df", term)
					case 5:
					fmt.Println(ColorRed,icon_warning,ColorReset)
					kass.CalcEpFrm(&mod)
				}
